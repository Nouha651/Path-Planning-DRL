...
def objective(trial):
    ...
    max_episodes = trial.suggest_int("max_episodes", 5500, 9000)  # Increased range for longer training
    max_timesteps = trial.suggest_int("max_timesteps", 2500, 4500)  # Increased upper limit
    learning_rate = trial.suggest_float("learning_rate", 5e-6, 3e-4)  # Reduced upper range for stability
    discount = trial.suggest_float("discount", 0.88, 0.96)  # Lowered range for short-term focus
    tau = trial.suggest_float("tau", 0.05, 0.15)  # Increased range to explore smoother parameter updates
    policy_noise = trial.suggest_float("policy_noise", 0.02, 0.1)  # Reduced upper limit for less aggressive noise
    noise_clip = trial.suggest_float("noise_clip", 0.03, 0.1)  # Reduced upper limit to limit extreme actions
    ...
    # Generate model file paths unique to the trial
    ...
    if os.path.exists(actor_model_path) and os.path.exists(critic_model_path):
        print(f"Loading saved model for fine-tuning (Trial {trial.number})...")
