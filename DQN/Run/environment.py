...
class PathPlanningEnv(gym.Env):
    ...
    def render(self, path=None, optimal_path=None):
        ...
        # Track if each label has been added to avoid duplicates in the legend
        labels_added = {"Overlap (DQN & RRT*)": False, "DQN Path": False, "RRT* Path": False}

        # Plot overlapping points in purple
        for position in overlap_points:
            label = "Overlap (DQN & RRT*)" if not labels_added["Overlap (DQN & RRT*)"] else ""
            ax.plot(position[0] + 0.5, position[1] + 0.5, 'o', color='purple', markersize=8, label=label)
            labels_added["Overlap (DQN & RRT*)"] = True

        # Plot DQN-only path points in blue
        for position in only_dqn_points:
            label = "DQN Path" if not labels_added["DQN Path"] else ""
            ax.plot(position[0] + 0.5, position[1] + 0.5, 'bo', markersize=8, label=label)
            labels_added["DQN Path"] = True

        # Plot RRT*-only path points in red
        for position in only_rrt_points:
            label = "RRT* Path" if not labels_added["RRT* Path"] else ""
            ax.plot(position[0] + 0.5, position[1] + 0.5, 'ro', markersize=8, label=label)
            labels_added["RRT* Path"] = True
